...
class HasRoleAndPermissionTest extends \TestCase
{
    ...
    public function testRolePermissions()
    {
        ...
        $this->assertEquals($permissions->toBase()->only([0, 1])->pluck('id')->toArray(), $user->rolePermissions()
                                                                                               ->get()
                                                                                               ->pluck('id')
                                                                                               ->toArray());
        ...
        $this->assertEquals($permissions->toBase()
                                        ->only([0, 1, 2, 3, 4, 5])
                                        ->pluck('id')
                                        ->toArray(), $user->rolePermissions()->get()->pluck('id')->toArray());
    }

    public function testHasRole()
    {
        $user = \Mockery::mock(User::class.'[hasOneRole]');
        $user->shouldReceive('hasOneRole')->with('role1')->once()->andReturn(true);
    ...
    public function testHasRole_all()
    {
        ...
        $user->shouldReceive('hasAllRoles')->with(['role1', 'role2'])->once()->andReturn(true);
    ...
    public function testHasOneRole_true()
    {
        ...
        $user->shouldReceive('checkRole')->once()->with('role1')->andReturn(false);

        $user->shouldReceive('checkRole')->once()->with('role2')->andReturn(true);
    ...
    public function testHasOneRole_false()
    {
        ...
        $user->shouldReceive('checkRole')->once()->with('role2')->andReturn(false);
    ...
    {
        $user = \Mockery::mock(User::class.'[checkRole]');
        $user->shouldReceive('checkRole')->once()->with('role1')->andReturn(true);
    ...
    public function testCheckRole()
    {
        ...
        $user->shouldReceive('getRoles')->once()->withNoArgs()->andReturn($roles);
    ...
    public function testHasPermission()
    {
        ...
        $user->shouldReceive('hasOnePermission')->with('permission1')->once()->andReturn(true);
    ...
    public function testHasPermission_all()
    {
        ...
        $user->shouldReceive('hasAllPermissions')->with(['permission1', 'permission2'])->once()->andReturn(true);
    ...
    public function testHasOnePermission_true()
    {
        ...
        $user->shouldReceive('checkPermission')->once()->with('permission1')->andReturn(false);

        $user->shouldReceive('checkPermission')->once()->with('permission2')->andReturn(true);
    ...
    public function testHasOnePermission_false()
    {
        ...
        $user->shouldReceive('checkPermission')->once()->with('permission2')->andReturn(false);
    ...
    {
        $user = \Mockery::mock(User::class.'[checkPermission]');
        $user->shouldReceive('checkPermission')->once()->with('permission1')->andReturn(true);
    ...
    public function testCheckPermission()
    {
        ...
        $user->shouldReceive('getPermissions')->once()->withNoArgs()->andReturn($permissions);
    ...
    public function testMagicCall()
    {
        ...
        //isMyRole
        $user->shouldReceive('hasRole')->once()->with('my.role')->andReturn(true);
        $this->assertTrue($user->callMagic('isMyRole', []));

        //canMyPermission
        $user->shouldReceive('hasPermission')->once()->with('my.permission')->andReturn(true);
